-- Wolt 
-- Assigment Task 2 a
-- Top 10 venues with highest margins and their average order size.
-- Average order size is calculated as average number of items in orders of each venue.
--
-- Author: Kennet Myllykoski
-- SQL: PostgreSQL 15

SELECT 
    c.venue_id,
    c.country, 
    ROUND(SUM(c.order_line_sales_total_ex_vat_eur)::numeric, 2) AS sales_per_venue_ex_vat_eur,
    ROUND(SUM(c.order_line_margin_ex_vat_eur)::numeric, 2) AS margin_per_venue_ex_vat_eur,
    ...
    ROUND((SUM(c.order_line_sales_total_ex_vat_eur) / COUNT(DISTINCT(c.purchase_id)))::numeric, 2) AS purchase_avg_eur,
    ROUND(SUM(c.count) / COUNT(DISTINCT(c.purchase_id))::numeric, 1) AS purchase_avg_count    
